...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/cartadminlist.css">
...
<body>
	...
	<form class="form-inline" action="adminorderList.do" method="post">
  	<select type class="form-control" id="sel1" name="searchCondition">
        <c:forEach items="${conditionMap}" var="option">
	        <option value="${option.value}">${option.key}</option>
        </c:forEach>        
    </select>
    <input class="form-control" type="text" name="searchKeyword" placeholder="검색어를 입력하세요.">
	...
	<c:forEach items="${adminlist}" var="adminlist">
		...
		<c:if test="${paging.nowPage > 1 && paging.lastBtn > paging.viewBtnCnt}">
			<li class="paylisypageing-item"><a class="adminpage-link" href="adminorderList.do?nowPage=${paging.nowPage-1}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">이전</a></li>
		</c:if>
		<c:forEach var="i" begin="${paging.startBtn}" end="${paging.endBtn}" step="1">
			<c:choose>
				...
				<c:otherwise><li class="adminpage-item"><a class="adminpage-link" href="adminorderList.do?nowPage=${i}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">${i}</a></li></c:otherwise>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.nowPage < paging.lastBtn  && paging.lastBtn > paging.viewBtnCnt}">
			<li class="adminpage"><a class="adminpage-link" href="adminorderList.do?nowPage=${paging.nowPage+1}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">이후</a></li>
		</c:if>
	</ul>
		</div>
