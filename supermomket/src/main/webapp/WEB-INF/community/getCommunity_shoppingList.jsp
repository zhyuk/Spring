...
<%@ include file="../view/menu.jsp"%>
		...
		<div class="container">
		<div class="h2">
			<h2 style="margin-top: 100px;">쇼핑정보</h2>
		</div>
			<nav id="searchNav" class="navbar navbar-expand-sm navbar-dark">
				<form class="form-inline" action="getCommunity_shoppingList.do" method="post" name="shopping">
			...
			<table class="table table-hover">
				...
				<tbody>
					<c:forEach items="${shoppingList}" var="shoppingList">
						<tr	onclick="shoppingList(${shoppingList.cm_no}, '${searchCondition}', '${searchKeyword}', ${paging.nowPage})" style="cursor: pointer;">
							<!-- 				<tr style="cursor:pointer;"> -->
							<td class="tdCenter"><a href="getShopping/${shoppingList.cm_no}">${shoppingList.cm_no}</a></td>
							<td class="title" style="text-align: left;">[쇼핑]${shoppingList.cm_title}</td>
							<td class="tdCenter">${shoppingList.cm_writer}</td>
							<td class="tdCenter">${shoppingList.cm_date}</td>
							<td class="tdCenter">${shoppingList.cm_view}</td>
							<td class="tdCenter">${shoppingList.cm_up }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			...
			<ul class="pagination">
				<c:if
					...
					<li class="page-item"><a class="page-link" href="getCommunity_shoppingList.do?nowPage=${paging.nowPage-1}">이전</a></li>
				</c:if>
				...
				<c:if
					...
					<li class="page-item"><a class="page-link"
						href="getCommunity_shoppingList.do?nowPage=${paging.nowPage+1}">다음</a></li>
				</c:if>
			</ul>
