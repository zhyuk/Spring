		...
		<div class="faqAdminmaindiv">
			...
			<div class="faqAdminsearchdiv">
				<form class="form-inline" action="getFaqAdminList.do" method="post">
					...
					<button type="button" id="faqadminbtn" class="faqadminbtn line"
						onclick="insertbtn()">글쓰기</button>
				</form>
			</div>
				...
				<tbody>
					<c:forEach items="${fvo}" var="fvo">
						<tr class="tradminCenter">
							...
							<td class="tdadminCenter"><button class="faqadminbtn"
									onclick="faqselTr(${fvo.faq_no})">글수정</button></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
				...
				<ul class="adminpagination">
					<c:if
						...
						<li class="adminpage-item"><a class="adminpage-link"
							href="getFaqAdminList.do?nowPage=${paging.nowPage-1}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">&lt;&lt;</a></li>
					</c:if>
					<c:forEach var="i" begin="${paging.startBtn}"
						end="${paging.endBtn}" step="1">
						<c:choose>
							<c:when test="${paging.nowPage==i}">
								<li class="adminpage-item active"><a class="adminpage-link">${i}</a></li>
							</c:when>
							<c:otherwise>
								<li class="adminpage-item"><a class="adminpage-link"
									href="getFaqAdminList.do?nowPage=${i}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if
