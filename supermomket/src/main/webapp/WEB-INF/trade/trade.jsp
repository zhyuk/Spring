...
<body>
	...
	<div class="container">
		<form id="filterForm"
			action="${pageContext.request.contextPath}/trade.do" method="post">
			<input type="hidden" name="sortCondition"
				value="${param.sortCondition}"> <input type="hidden"
				name="categories" value="${selectedCategories}"> <input
			...
			<div class="content-wrapper">
				...
				<div class="filter-section" id="filterSection">
					...
					<div class="filter-options">
						<span
							class="filter-option ${param.sortCondition == 'latest' || param.sortCondition == null ? 'active' : ''}"
							data-sort="latest">최신순</span> <span
							class="filter-option ${param.sortCondition == 'lowPrice' ? 'active' : ''}"
							data-sort="lowPrice">낮은가격순</span> <span
							class="filter-option ${param.sortCondition == 'highPrice' ? 'active' : ''}"
					...
					<div class="status-filter">
						...
						<div class="filter-options">
							<span
								class="filter-option ${param.status == null ? 'active' : ''}"
								data-status="">전체</span> <span
								class="filter-option ${param.status == 'SELLING' ? 'active' : ''}"
								data-status="SELLING">판매중</span> <span
								class="filter-option ${param.status == 'RESERVED' ? 'active' : ''}"
								data-status="RESERVED">예약중</span> <span
								class="filter-option ${param.status == 'COMPLETED' ? 'active' : ''}"
					...
					<div class="category-filter">
						...
						<ul class="category-list">
							<li class="category-option"><input type="checkbox"
								id="motorcycle" name="category" value="오토바이"
								${selectedCategories.contains('오토바이') ? 'checked' : ''}>
								<label for="motorcycle">오토바이</label></li>
							<li class="category-option"><input type="checkbox"
								id="electronics" name="category" value="전자기기"
								${selectedCategories.contains('전자기기') ? 'checked' : ''}>
								<label for="electronics">전자기기</label></li>
							<li class="category-option"><input type="checkbox"
								id="clothing" name="category" value="의류"
								${selectedCategories.contains('의류') ? 'checked' : ''}> <label
								for="clothing">의류</label></li>
							<li class="category-option"><input type="checkbox"
								id="accessories" name="category" value="잡화"
								${selectedCategories.contains('잡화') ? 'checked' : ''}> <label
								for="accessories">잡화</label></li>
							<li class="category-option"><input type="checkbox"
								id="furniture" name="category" value="가구/인테리어"
								${selectedCategories.contains('가구/인테리어') ? 'checked' : ''}>
								<label for="furniture">가구/인테리어</label></li>
							<li class="category-option"><input type="checkbox" id="baby"
								name="category" value="출산/유아동"
								${selectedCategories.contains('출산/유아동') ? 'checked' : ''}>
								<label for="baby">출산/유아동</label></li>
							<li class="category-option"><input type="checkbox"
								id="sports" name="category" value="스포츠/레저"
								${selectedCategories.contains('스포츠/레저') ? 'checked' : ''}>
								<label for="sports">스포츠/레저</label></li>
							<li class="category-option"><input type="checkbox"
								id="beauty" name="category" value="뷰티/미용"
								${selectedCategories.contains('뷰티/미용') ? 'checked' : ''}>
								<label for="beauty">뷰티/미용</label></li>
						</ul>
					</div>
				</div>
				...
				<div class="main-content">
					<div class="search-input-wrapper">
						...
						<button type="button" class="filter-toggle" id="filterToggle">필터</button>
					</div>
					...
					<div class="header">
						...
						<c:choose>
							...
							<c:otherwise>
								<a class="write-btn"
									href="${pageContext.request.contextPath}/trade_write.do">글쓰기</a>
							</c:otherwise>
						</c:choose>
					</div>
					...
					<div class="grid">
						<c:forEach var="trade" items="${tradeList}">
							<a
								href="${pageContext.request.contextPath}/trade_detail.do?t_no=${trade.t_no}"
								class="item-link">
								<div class="item">
									<div class="image-container ${trade.t_status}">
										...
										<img
											...
											alt="상품 이미지">
										<c:if
											test="${trade.t_status eq 'RESERVED' or trade.t_status eq 'COMPLETED'}">
											<div class="status-overlay">${trade.t_status eq 'RESERVED' ? '예약중' : '거래완료'}
											</div>
										</c:if>
									</div>
									<div class="item-info">
										...
										<div class="item-price">
											<fmt:formatNumber value="${trade.t_price}" pattern="#,###" />
											원
										</div>
					...
					<div class="pagination">
						<c:if test="${currentPage > 1}">
							<a
								href="?page=${currentPage-1}&sortCondition=${param.sortCondition}&categories=${param.categories}&searchKeyword=${param.searchKeyword}&status=${param.status}"
								class="page-link">&lt;</a>
						</c:if>

						<c:forEach begin="1" end="${totalPages}" var="pageNum">
							<a
								href="?page=${pageNum}&sortCondition=${param.sortCondition}&categories=${param.categories}&searchKeyword=${param.searchKeyword}&status=${param.status}"
