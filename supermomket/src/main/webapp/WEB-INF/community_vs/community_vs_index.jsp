						...
						<select name="searchSelect">
							<c:forEach items="${conditionMap}" var="option">
								<option value="${option.value}"
									<c:if test="${searchCondition==option.value}">selected</c:if>>${option.key}
								</option>
							</c:forEach>
				...
				<table>
					...
					<tbody>
						<c:forEach items="${boardList}" var="board">
							<tr onclick="selBoard(${board.vs_no}, '${searchCondition}', '${searchKeyword}', ${paging.nowPage})" class="pointer">
								<td class="number">${board.vs_rownumber}</td>
								<td class="title"><span>${board.vs_title}</span> 
