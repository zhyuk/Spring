						...
						<div class="text-group board_info">
							...
							<c:if test="${sessionScope.userId == board.vs_writer}">
								<div class="btn-area">
									...
									<button class="pointer"
										onclick="deleteBoard(${board.vs_no}, '${searchCondition}', '${searchKeyword}', ${nowPage})">삭제</button>
								</div>
							</c:if>
						...
						<div class="text-group board_content_box">
							...
							<ul class="vote_box">
								...
								<c:if test="${userId != null}">
									<li><button class="pointer"
											onclick="vote(${board.vs_no}, 1)">투표하기</button></li>
									<li><button class="pointer"
											onclick="vote(${board.vs_no}, 2)">투표하기</button></li>
								</c:if>
							</ul>
						...
						<c:if test="${comment.vs_rcno == 0}">
							<div class="comment_box">
								<div class="comment_info">
									...
									<button class="recomment_btn pointer"
										onclick="viewRecomment(${comment.vs_cno})">답글보기</button>
								...
								<div class="comment_btn">
									<c:if test="${userId != null && userId == comment.vs_writer}">
										...
										<button type="button" class="updateCancelButton pointer"
											onclick="updateCommentCancel()">취소</button>
									</c:if>
								</div>
								<div class="clear"></div>
											...
											<div class="comment_btn">
												<c:if
													...
													<button type="button" class="deleteCommentButton"
														onclick="deleteComment(${comment.vs_no}, ${recomment.vs_cno})">삭제</button>
													<button type="button" class="updateCancelButton"
														onclick="updateCommentCancel()">취소</button>
												</c:if>
											</div>
											<div class="clear"></div>
